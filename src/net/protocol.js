...
export const NET_RATE = {
  movingHz: 20, idleHz: 8, snapshotHz: 20,
  /**
   * この時間だけ入力が途切れたら、その参加者を止める。
   * 参加者は待機中 8Hz(=125ms間隔) でしか送らないので、短すぎるとホスト側の
   * 軽い処理落ち（アセット読み込み・GC）だけで全員がカクつく。余裕を持たせる。
   */
  inputTimeoutMs: 1000,
  /** 1フレームで進めてよい上限(s)。長いフレームで参加者がワープしないようにする。 */
  maxRemoteStep: 0.1,
  maxPlayers: 16, bossHpPerPlayer: 0.45,
}
...
export const iceServers = () => {
  ...
  // 公開STUNは応答しないことがあるので複数並べる。TURNは環境変数からのみ注入する。
  const fallback = [{ urls: [
