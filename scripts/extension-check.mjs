...
try {
  ...
  const fire = await evaluate(`({ hint:!!document.querySelector('#marugoto-home-spell-hint'), effect:!!document.querySelector('#marugoto-home-fire-overlay'), hiddenCanvases:[...document.querySelectorAll('canvas')].filter((x)=>x.style.opacity==='0').length })`)
  check('Cキーで火球の視覚演出を開始', fire.hint && fire.effect, JSON.stringify(fire))
  check('元のThree.js Canvasを隠さない', fire.hiddenCanvases === 0, JSON.stringify(fire))
  const dimensions = await evaluate(`(() => { const c=[...document.querySelectorAll('canvas')].find((x)=>{const r=x.getBoundingClientRect(); return r.width>120&&r.height>100&&r.bottom>0&&r.top<innerHeight}); const r=c?.getBoundingClientRect(); return r&&[r.width,r.height] })()`)
